...
public class EventCenter extends Handler implements Runnable {
    ...
    private List<OnEventListener> collectList = null;
    ...
    public EventCenter() {
        ...
        collectList = new ArrayList<OnEventListener>();
    ...
    public void addEventListener(String eventType, OnEventListener callback) {
        addEventListener(eventType, callback, false);
    }

    public void addEventListener(String eventType, OnEventListener callback, boolean autoCollect) {
        ...
        if (msg.what == CENTER_MSG) {
            ...
            if (eventMap.containsKey(e.type)) {
                for (OnEventListener callback : eventMap.get(e.type)){
                    ...
                    if(collectList.contains(callback)){
                        removeEventListener(e.type, callback);
                        collectList.remove(callback);
                    }
                }
            }
        }
