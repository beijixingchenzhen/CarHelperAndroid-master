...
public class Utils {
    ...
    static public String tsTransFromServer(String value) {
    ...
    static public String timestampToDisplay(long ts, int format) {
        try {
            ...
            if (format == FULL_TIME_STRING) {
                ret = calendar.get(Calendar.YEAR) + " " + (calendar.get(Calendar.MONTH) + 1) + "-" +
                        calendar.get(Calendar.DAY_OF_MONTH) + "" + period + "" + hours + "点";
    ...
    public static void safeSetSrc(ImageView view, int src){
        if(view != null)
            view.setImageResource(src);
    }

    public static boolean showOrGone(boolean cond, View view){
        if(view == null)
            return false;
        if(cond){
    ...
    public static void showToast(Context c, String text, boolean isLong) {
    ...
    public static void setPermanentGlobalData(Context c, String key, String val) {
        SharedPreferences preferences = c.getSharedPreferences("Permanent", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        editor.putString(key, val);
        editor.commit();
    }

    public static String gettPermanentGlobalData(Context c, String key) {
        SharedPreferences preferences = c.getSharedPreferences("Permanent", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        String val = preferences.getString(key, "");
        return val;
...
//        return DistanceUtil.getDistance(l1.getPoint(), l2.getPoint())/1000;
//    }
//
//    public static double calDistanceOfPoint(double lat1, double lon1, double lat2, double lon2){
//        return DistanceUtil.getDistance(new LatLng(lat1, lon1), new LatLng(lat2, lon2));
//    }
//
//    public static void phoneCall(String phoneNum){
//        if(BaseActivity.currActivity != null){
//            if(phoneNum.length() > 0){
//                Intent intent = new Intent(Intent.ACTION_CALL, Uri.parse("tel:" + phoneNum));
//                BaseActivity.currActivity.startActivity(intent);
//            }else{
//                Utils.showToast(BaseActivity.currActivity.getApplicationContext(), "电话号码为空!");
//            }
//        }
//    }
